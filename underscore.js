...
// Bonus Points
const totalByMonth = _.map(purchasesByMonth, purchasesInMonth => {
  let month = _.map(purchasesInMonth, "price");

  return _.reduce(month, (memo, next) => memo + next, 0);
});
...
let getJedi = _.memoize(n => axios.get(`https://swapi.co/api/people/${n}`));
